    ...
    try {
      ...
      if (action === 'getRecordList') {
        ...
        const list = sortRecords(
          filterRecords(response.data || [], payload).map(
            normalizeRecordForList
          )
        )
      ...
      if (action === 'getMediaDisplayUrls') {
        ...
        const fileIds = normalizeRequestedMediaFileIds(payload.fileList).filter(
          (fileId) => accessibleFileIds.has(fileId)
        )
      ...
      if (isMissingRecordsCollectionError(error)) {
        return {
          success: false,
          message:
            '云数据库集合 records 未创建，请先在云开发数据库中创建 records 集合',
          data: {
            list: [],
            total: 0,
          },
...
function isMissingRecordsCollectionError(error) {
  ...
  return /DATABASE_COLLECTION_NOT_EXIST|collection not exists|Db or Table not exist: records|-502005/.test(
    message
  )
}
...
async function prepareMediaReview(payload, downloadFile) {
  ...
  for (const item of payload.mediaList) {
    ...
    try {
      ...
      const fileContent =
        response && response.fileContent ? response.fileContent : response
...
async function createPendingMediaReview({
  ...
  if (
    typeof checkMedia !== 'function' ||
    typeof getTempFileURL !== 'function'
  ) {
    return failure(CONTENT_CHECK_FAILED_MESSAGE)
  }
...
function createMediaCheckError(response) {
  const error = new Error(
    getErrorMessage(response) ||
      'media security check did not return a trace id'
  )
...
function getErrorMessage(source) {
  return (source && (source.errMsg || source.errmsg || source.message)) || ''
}
...
async function getUploadReviewDecision({
  ...
  const tasks = await Promise.all(
    imageMedia.map((item) => getRecordById(contentSecurityTasks, item.reviewId))
  )

  if (
    tasks.some(
      (task, index) => !isUploadReviewForMedia(task, imageMedia[index], openid)
    )
  ) {
    return 'missing'
  }
...
function isUploadReviewForMedia(task, media, openid) {
  const mediaList =
    task && task.payload && Array.isArray(task.payload.mediaList)
...
function normalizeRequestedMediaFileIds(fileList) {
  return Array.from(
    new Set(
      (Array.isArray(fileList) ? fileList : []).filter(
        (fileId) => typeof fileId === 'string' && isCloudFileId(fileId)
      )
    )
